...
[Scenario Lab 阶段性交接](project-materials/SCENARIO_LAB_HANDOFF_2026-08-15.md) 是一次真实 API 测试的历史快照：该次“热汤与门口的外婆”测试因关键帧连续两轮未通过而停在 Stage 04，没有提交 H3 视频。之后人工批准并制作完成的场景资产与过程记录，以 `project-materials/docs/影游生产记录/`、`data/` 和 `videos/` 中的当前文件为准。

## VR180

`VR180/` 是在线发布运行时，`project-materials/VR180/` 是筛选后的制作与 QA 记录。当前运行时支持：
...
制作标准与当前选择依据见 [VR180 Production Standard](project-materials/VR180/VR180_PRODUCTION_STANDARD.md) 和 [VR180 开发现状](project-materials/VR180/README.md)。未经新的明确授权，不应继续提交付费 fal.ai 任务。

## 开发与研究材料

- [Scenario Lab 工作流规范](project-materials/docs/Scenario_Lab/Scenario_Lab工作流规范.md)；
- [Scenario Lab 开发提示词](project-materials/docs/Scenario_Lab/开发提示词.md)；
- [Family 01–02 制作过程记录](project-materials/docs/影游生产记录/Family_01-Scenario_02_制作过程记录.md)：从脚本到 H3 视频的完整过程；
- [Family 01–03 制作过程记录](project-materials/docs/影游生产记录/Family_01-Scenario_03_制作过程记录.md)：四视频、三选择流程和修订证据；
- `project-materials/docs/界面参考/`：界面方向、封面和历史截图；
- `project-materials/VR180/`：筛选后的 VR180 prompts、manifest 与 QA 证据；
- `source/supabase/schema.sql`：共享体验记录、研究者 allowlist 与 RLS；
- `source/tests/`：Prompt compiler 与静态构建行为验证。

GitHub 发布仓库为保持现有 Pages 地址不变，根目录继续保存可运行静态站；可读源码快照位于 [source/](source/)，可公开开发材料位于 [project-materials/](project-materials/)。密钥、本地缓存、日志、被忽略的构建目录及超过 GitHub 单文件限制的原始制作母版不会上传。
...
1. 在 Supabase SQL Editor 执行 `source/supabase/schema.sql`；
